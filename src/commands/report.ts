...
interface ReportFinding {
  matched_task?: { id: string; title: string; description: string };
  new_task?: { title: string; description: string; priority: string; type: string };
  analysis: string;
}
...
export async function handleReport(
  ...
  try {
    const res = await fetch(`${apiUrl}/api/v1/agents/memories/shared`, {
  ...
  // Also fetch active sprint tasks
  let activeTasks: Task[] = [];
  try {
    activeTasks = await api.fetchTasks();
  } catch { /* non-fatal */ }
  ...
  s.stop(`Found: ${sharedMemories.length} knowledge entries, ${backlogTasks.length + activeTasks.length} tasks, ${errorLogs ? "error logs" : "no error logs"}`);
